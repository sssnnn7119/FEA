...
def generate_shell_from_surface(
        fe: FEA_Main, surface_names: list[str], shell_thickness: float, surface_new_name: Optional[list[str]] = None
    ...
    unique_node_indices = torch.unique(
        surface_elems
    ...
    v0 = fe.nodes[surface_elems[:, 0]]
    v1 = fe.nodes[surface_elems[:, 1]]
    v2 = fe.nodes[surface_elems[:, 2]]
    ...
    # Calculate averaged normals for each node using sparse tensor operations
    ...
    node_idx_map = torch.full((max_node_idx + 1, ),
                              ...
                              dtype=torch.int64)
    node_idx_map[unique_node_indices] = torch.arange(
        unique_node_indices.shape[0], device=fe.nodes.device)
    ...
    cols = torch.cat(
        [torch.arange(surface_elems.shape[0], device=fe.nodes.device)] * 3)
    ...
    node_triangle_map = torch.sparse_coo_tensor(
        sparse_indices,
        values,
        size=(unique_node_indices.shape[0], surface_elems.shape[0]),
        device=fe.nodes.device)

    # Multiply the sparse matrix by the normals to get the sum of normals for each node
    node_normals = torch.sparse.mm(
        node_triangle_map,
        normals)  # Normalize the averaged normals (vectorized)
    normal_lengths = torch.norm(node_normals, dim=1, keepdim=True)
    mask = normal_lengths > 1e-10  # Avoid division by zero
    node_normals = torch.where(mask, node_normals / normal_lengths,
                               node_normals)

    # Create new nodes by offsetting the original nodes by the thickness (vectorized)
    new_nodes = fe.nodes[unique_node_indices] + node_normals * shell_thickness
