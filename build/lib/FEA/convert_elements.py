...
def fast_edge_lookup(edge_dict, edges_tensor):
    """
...
def convert_elements_batch(fe: FEA_Main, element_names_to_convert: list[str]=None):
    """
    Convert multiple element types to second-order elements in a single operation.
    
    This function converts C3D4 elements to C3D10 elements and C3D6 elements to C3D15 elements.
    
    Args:
        fe (FEA.Main.FEA_Main): The FEA model instance containing elements to convert
        element_names_to_convert (list, optional): List of element set names to convert.
                                                These are names as they appear in the fe.elems dictionary,
                                                such as 'element-0' or 'shell_elements'.
                                                If None, converts all compatible elements.
                                        
    Returns:
        FEA.Main.FEA_Main: Updated FEA model with converted elements
    """
    # Call the main conversion function with the specified element names
    return convert_to_second_order(fe, element_names_to_convert)
