...
class FEA_Main():
    ...
    def solve(self, RGC0: torch.Tensor = None, tol_error: float = 1e-7):
        """
        ...
        while True:
            ...
            if not self._solve_iteration(RGC=self.RGC,
                                         tol_error=tol_now,
    ...
    def _solve_iteration_opt(self,
        ...
        def closure(x):
            R = -self._assemble_Stiffness_Matrix(
                RGC=self._GC2RGC(x + self.GC),
                load_percentage=load_percentage)[0]
            return R
