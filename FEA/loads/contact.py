...
class ContactBase(BaseLoad):
    ...
    def _calculate_penalty_terms(self, y1: torch.Tensor, n1: torch.Tensor, y2: torch.Tensor, n2: torch.Tensor):
        """
        Calculate penalty terms for contact.
        
        Args:
            y1, N1: Positions and normals for first surface
            y2, N2: Positions and normals for second surface
            
        Returns:
            torch.Tensor: Combined penalty values
        """
        
        dy = y1[:, None, self._point_pairs[0], :] - y2[None, :, self._point_pairs[1], :]
        dn = n1[:, None, self._point_pairs[0], :] - n2[None, :, self._point_pairs[1], :]
        ndot = (n1[:, None, self._point_pairs[0], :] * n2[None, :, self._point_pairs[1], :]).sum(dim=-1)

        # Calculate f penalty term
        MM = (self._penalty_start_f - ndot) / (self._penalty_start_f - self._penalty_end_f)
        MM = MM.clamp(0, 1)
        f = MM**3 * (6*MM**2 - 15*MM + 10)
        
        # Calculate g penalty term
        D = self._penalty_distance_g + (dn * dy).sum(dim=-1) / 2
        D[D < 0] = 0
        g = (D / self._penalty_factor_g) ** self._penalty_degree

        # Calculate h penalty term
        L = dy.norm(dim=-1)
...
class ContactSelf(ContactBase):
    ...
    def _filter_point_pairs(self, surface_element1, surface_element2, nodes):
        ...
        # Get surface normals
        normal1 = surface_element1.get_gaussian_normal(self._fea.nodes)
        normal2 = surface_element2.get_gaussian_normal(self._fea.nodes)
        elems_gaussian1 = surface_element1.gaussian_points_position(self._fea.nodes)
        elems_gaussian2 = surface_element2.gaussian_points_position(self._fea.nodes)
    ...
    def get_potential_energy(self, RGC):
        ...
        # Calculate positions and normals once for efficiency
        y, N, _, _ = self._calculate_positions_and_normals(RGC, self.surface_element)
        n = N / N.norm(dim=-1, keepdim=True)

        # Calculate penalty terms
        penalty = self._calculate_penalty_terms(y, n, y, n)
        
        # Compute the potential energy
        potential_energy = (self._ratio * penalty).sum()
    ...
    def get_stiffness(self, RGC):
        ...
        # pdU_2 = torch.sparse_coo_tensor(pdU_2_indices.reshape([2, -1]), pdU_2_values.flatten(), size=Y.shape*2)
...
class Contact(ContactBase):
    ...
    def initialize(self, fea):
        ...
        self.surface_element1 = fea.surface_sets.get_elements(self.surface_name1)[0]
        self.surface_element2 = fea.surface_sets.get_elements(self.surface_name2)[0]

        # Filter point pairs between the two surfaces
        self._filter_point_pairs(
            self.surface_element1, self.surface_element2, fea.nodes)
        
    def get_potential_energy(self, RGC):
        self._filter_point_pairs(self.surface_element1, self.surface_element2, self._fea.nodes + RGC[0])
        # Calculate positions and normals for both surfaces
        y1, N1, y2, N2 = self._calculate_positions_and_normals(RGC, self.surface_element1, self.surface_element2)
        n1 = N1 / N1.norm(dim=-1, keepdim=True)
        n2 = N2 / N2.norm(dim=-1, keepdim=True)

        # Calculate penalty terms
        penalty = self._calculate_penalty_terms(y1, n1, y2, n2)
    ...
    def get_stiffness(self, RGC):
        ...
        # Calculate positions and normals for both surfaces
        Ye1 = Y[self.surface_element1._elems]
        Ye2 = Y[self.surface_element2._elems]
        ...
        E1 = torch.zeros([num_g1, num_p, 2, 3], device=U.device)
        E1[:, :, 0] = y1[:, self._point_pairs[0]]
        E1[:, :, 1] = n1[:, self._point_pairs[0]]

        E2 = torch.zeros([num_g2, num_p, 2, 3], device=U.device)
        ...
        pdU_indices1 = torch.stack([pdU_indices1*3, pdU_indices1*3+1, pdU_indices1*3+2], dim=-1)
        
        pdU_indices2 = self.surface_element2._elems[tri_ind[1]].to(torch.int64)
        pdU_indices2 = torch.stack([pdU_indices2*3, pdU_indices2*3+1, pdU_indices2*3+2], dim=-1)
        
        pdU_indices = torch.cat([pdU_indices1.flatten(), pdU_indices2.flatten()], dim=0).to(self._fea.nodes.device)
